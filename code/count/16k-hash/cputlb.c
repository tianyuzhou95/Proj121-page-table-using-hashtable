...
#ifdef TEST_TIME
static double mymsecond(struct timespec time1,struct timespec time2)
{
    return ( (double) (time2.tv_sec - time1.tv_sec) * 1.e6 + (double) (time2.tv_nsec-time1.tv_nsec) * 1.e-3);
}
#endif
...
char filename[50] = "/home/oscomp/proj121-code/16k-hash/test";
extern double interval_test;
extern unsigned long all_cnt;
extern unsigned long mmu_cnt;
extern unsigned long load_helper_cnt;
extern unsigned long store_helper_cnt;
extern bool is_test;
...
tb_page_addr_t get_page_addr_code(CPUArchState *env, target_ulong addr)
{
    ...
    tb_page_addr_t tmp = get_page_addr_code_hostp(env, addr, NULL);
    #ifdef TEST_TIME
    if(is_test){
        clock_gettime(CLOCK_REALTIME, &time2);
        interval_test += mymsecond(time1,time2);
    }
    #endif
    return tmp;
}
