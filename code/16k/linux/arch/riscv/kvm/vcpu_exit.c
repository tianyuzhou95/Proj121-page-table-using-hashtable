			     ...
			     struct kvm_cpu_trap *trap)
{
	...
	// kvm_info("fault_addr:%lx\n",fault_addr);
	// kvm_info("gfn:%lx\n",gfn);											//GPA
	// kvm_info("memslot.userspace_addr:0x%lx\n",memslot->userspace_addr);	
	// kvm_info("memslot.npages:%ld\n",memslot->npages);
	// kvm_info("hva:0x%lx\n",hva);											//HVA
	...
	// kvm_info("enter into kvm_riscv_stage2_map\n");
	ret = kvm_riscv_stage2_map(vcpu, memslot, fault_addr, hva,
		(trap->scause == EXC_STORE_GUEST_PAGE_FAULT) ? true : false);
	if (ret < 0){
		kvm_err("error in kvm_riscv_stage2_map\n");
		return ret;
	}
	return 1;
}
			...
			struct kvm_cpu_trap *trap)
{
	...
	// kvm_info("trap->scause = %lx",trap->scause);
