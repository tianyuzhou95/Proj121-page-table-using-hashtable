...
{
	set_kernel_memory(__start_rodata, _data - PMD_SIZE, set_memory_ro);
	if (IS_ENABLED(CONFIG_64BIT))
...
asmlinkage void __init setup_vm(uintptr_t dtb_pa)
{
	...
	kernel_map.virt_addr = KERNEL_LINK_ADDR - 0x200000;
...
#else
	kernel_map.phys_addr = (uintptr_t)(&_start) - 0x200000;
	...
	kernel_map.va_pa_offset = PAGE_OFFSET - (kernel_map.phys_addr + 0x200000);
...
void __init misc_mem_init(void)
{
	...
	arch_numa_init();
	sparse_init();
	zone_sizes_init();	//*out_start = 9ffabffc 
#ifdef CONFIG_KEXEC_CORE
	reserve_crashkernel();
#endif
	memblock_dump_all();
