				  ...
				  u32 *ptep_level)
{
    ...
    unsigned long hashIdx = blockNum & 0x3fff; //取后14位作为idx
    unsigned long blockOff = (unsigned long)addr << 49 >> 61;
    //获取该行的tag，需要进行比对是否一致
	unsigned long curLine = (hashIdx * 64);
	...
	while (1) {
		startTag = tmpPgd + curLine;
        tag = *((unsigned long *)startTag);
        startTable = startTag + blockOff * 7 + 8;
        valid = tag >> 56;
        if (valid) {
...
{
	unsigned long size = PAGE_SIZE * 256;
